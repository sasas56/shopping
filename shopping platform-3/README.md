...
│   │   └── index.ts              # 数据类型
│   ├── utils/                    # 工具函数
│   │   └── request.ts            # HTTP 请求封装（独立请求工具，含 Mock 路由）
│   ├── mock/                     # Mock 数据与路由
│   │   ├── data.ts               # Mock 数据源与 handler
│   │   └── handler.ts            # Mock 路由注册表
│   ├── views/                    # 页面组件
│   │   ├── Home.vue             # 首页
...
商品列表的分页查询、分类筛选、关键字搜索均通过独立请求工具 `src/utils/request.ts` 完成，开发环境默认启用 Mock 数据（访问一页发送一次请求）。详细实现思路请参考 [异步请求与Mock实现说明](docs/异步请求与Mock实现说明.md)。
